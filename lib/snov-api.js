...
function httpsRequest(options, postData) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(20000, () => req.destroy(new Error('Timeout: sem resposta da API em 20s')));
    req.on('error', reject);
...
class SnovClient {
  ...
  async getAllActivity(campaignId, from, to) {
    // A API do Snov.io trata "date_to" como o INÍCIO daquele dia (exclusivo),
    // não o fim — eventos que acontecem no próprio dia "to" ficam de fora.
    // Por isso buscamos um dia a mais e filtramos localmente pelo intervalo
    // exato pedido, usando o horário real de cada evento.
    const toPlus1 = new Date(to + 'T00:00:00Z');
    toPlus1.setUTCDate(toPlus1.getUTCDate() + 1);
    const toPlus1Str = toPlus1.toISOString().slice(0, 10);

    const all = [];
    for (const [chunkFrom, chunkTo] of this.splitInto31DayChunks(from, toPlus1Str)) {
      let offset = 0;
      const count = 100;
      while (true) {
    ...
    return all.filter(e => {
      ...
      const t = new Date(e.event_time.replace(' ', 'T') + 'Z').getTime();
